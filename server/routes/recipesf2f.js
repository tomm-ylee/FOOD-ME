...
API_DOMAIN = "http://food2fork.com/api"
...
router.get('/search/:searchPhrase', function(req, res, next) {
  ...
  // const SearchURL = `${API_DOMAIN}/search?key=${API_KEY}&q=${searchPhrase}`
  request(
    `${API_DOMAIN}/search?key=${API_KEY}&q=${searchPhrase}`, // Use %20 for space, and use "," to separate ingredients
...
//   const recipeId = req.params.id;
//   request(
//     `${API_DOMAIN}/get?key=${API_KEY}&rId=${recipeId}`,
//     (err, apiResponse, data) => {
//       res.send(apiResponse.body)
//     }
//   )
